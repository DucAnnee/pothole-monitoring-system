...
# Severity calculation weights (configurable)
...
def calculate_severity_score(depth_cm: float, surface_area_cm2: float) -> int:
    ...
    Formula: severity = clip[MIN_SCORE, MAX_SCORE]( ceil(AREA_WEIGHT * area_discrete + DEPTH_WEIGHT * depth_discrete) )

    Both area and depth are first mapped to discrete values 1-10,
    then combined with configurable weights (default: area: 0.6, depth: 0.4).
    The result is rounded up (ceil) and clipped to [1, 10].
    ...
    # Apply ceiling and clip to [MIN_SCORE, MAX_SCORE]
    severity_score = max(MIN_SCORE, min(MAX_SCORE, math.ceil(raw_score)))
...
def get_severity_level(severity_score: int) -> str:
    ...
    if severity_score < THRESHOLDS.get("minor_max", 3.25):
        return "MINOR"
    elif severity_score < THRESHOLDS.get("moderate_max", 5.5):
        return "MODERATE"
    elif severity_score < THRESHOLDS.get("high_max", 7.75):
        return "HIGH"
    else:
        return "CRITICAL"
