...
# Import latency tracker
import sys
import os
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'shared'))
try:
    from latency_tracker import record_pipeline_latency, PipelineLatencyEvent, get_tracker
    LATENCY_TRACKING_ENABLED = True
except ImportError:
    LATENCY_TRACKING_ENABLED = False
    print("[WARN] Latency tracking module not found, disabled")
...
# Input topics
RAW_EVENTS_TOPIC = config.raw_events_topic
SEVERITY_SCORE_TOPIC = config.severity_score_topic
...
# H3 Resolutions
H3_DEDUP_RESOLUTION = config.h3_dedup_resolution
H3_CACHE_RESOLUTION = config.h3_cache_resolution
...
OSM_MIN_REQUEST_INTERVAL = config.osm_min_request_interval

# Aggregation settings - Now configurable via YAML/env vars
# Default: 300 seconds (5 minutes) to wait for both raw event AND severity score
AGGREGATION_TIMEOUT_SECONDS = config.aggregation_timeout_seconds
    ...
    def cleanup_stale(self, max_age_seconds: float) -> int:
        ...
        with self._lock:
            ...
            for key in stale_keys:
                ...
                # Log timeout warning if configured
                if LOG_TIMEOUT_WARNINGS:
                    if has_raw and not has_sev:
                        print(f"[TIMEOUT WARNING] Event {key} timed out after {age_seconds:.1f}s waiting for severity score. "
                              f"Consider increasing AGGREGATION_TIMEOUT_SECONDS (current: {max_age_seconds}s)")
                    elif has_sev and not has_raw:
                        print(f"[TIMEOUT WARNING] Event {key} timed out after {age_seconds:.1f}s waiting for raw event. "
                              f"This is unusual - check if raw events are being produced correctly.")
                
                print(f"[CLEANUP] Dropping {key}: raw={has_raw}, severity={has_sev}, age={age_seconds:.1f}s")
                del self._store[key]
                removed += 1
...
class FinalEnrichmentService:
    ...
    def _record_latency(
        ...
        """Record pipeline latency metrics to Redis."""
        try:
            # Get timestamps from events
            edge_detected_at = raw_event['timestamp']  # Edge detection time (ms)
            severity_calculated_at = severity_event['calculated_at']  # When severity was calculated (ms)
            pothole_stored_at = int(time.time() * 1000)  # Now (ms)
            
            # Record latency
            event = record_pipeline_latency(
                event_id=event_id,
            ...
            if event.total_pipeline_ms:
                print(f"[LATENCY] Total pipeline: {event.total_pipeline_ms}ms (edge→severity: {event.raw_to_severity_ms}ms)")
        except Exception as e:
            print(f"[WARN] Failed to record latency: {e}")
