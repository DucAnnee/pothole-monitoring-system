...
def apply_sam3_nms(
    ...
    if max_detections > 0 and len(filtered_scores) > max_detections:
        top_k_scores, top_k_indices = torch.topk(
            filtered_scores, k=max_detections, largest=True
        )
...
def convert_predictions_to_coco_format(
    ...
    if merge_cracks:
        ...
        print(
            f"[INFO] Using CRACK MERGING mode: prob_threshold={prob_threshold}, merge_iou={merge_iou_threshold}, max_dets={max_detections}"
        )
        print(
            f"[INFO] This will MERGE overlapping crack segments instead of suppressing them"
        )
    else:
        print(
            f"\n[INFO] Converting {len(predictions_list)} predictions to COCO format..."
        ...
        print(
            f"[INFO] Using SAM3 NMS: prob_threshold={prob_threshold}, nms_iou={nms_iou_threshold}, max_dets={max_detections}"
        )

    for img_id, preds in tqdm(
        zip(image_ids, predictions_list),
        ...
        if merge_cracks:
            ...
            if len(filtered_masks) > 0:
                ...
                filtered_masks = (
                    merged_masks.float()
                )  # Already binary, just convert to float
        ...
        if len(filtered_masks) > 0:
            ...
            for idx, (rle, score, box) in enumerate(
                zip(rles, filtered_scores.cpu().tolist(), filtered_boxes.cpu().tolist())
            ):
                cx, cy, w, h = box
                x = (cx - w / 2) * resolution
                y = (cy - h / 2) * resolution
...
def create_coco_gt_from_dataset(dataset, image_ids=None, mask_resolution=288):
    ...
    print(
        f"[INFO] Created {len(coco_gt['images'])} images, {len(coco_gt['annotations'])} annotations"
    )
...
def validate(
    ...
    if use_base_model:
        ...
        if config_path is None or weights_path is None:
            raise ValueError(
                "config_path and weights_path are required when use_base_model=False"
            )
        ...
        # Get batch_size and resolution from config
        batch_size = config["training"]["batch_size"]
        dataset_resolution = int(config.get("training", {}).get("resolution", 1008))
    ...
    class DirectCOCODataset(COCOSegmentDataset):
        def __init__(self, data_dir, resolution=1008):
            self.data_dir = Path(data_dir)
    ...
    # Pass resolution into dataset (default 1008)
    if "dataset_resolution" not in locals():
        dataset_resolution = 1008
    print(f"Using dataset resolution: {dataset_resolution}x{dataset_resolution}")
    val_ds = DirectCOCODataset(val_data_dir, resolution=dataset_resolution)
    ...
    with torch.no_grad():
        for batch_idx, batch_dict in enumerate(tqdm(val_loader, desc="Validation")):
            ...
            # print("Image batch shape:", input_batch.img_batch.shape)
            # print("Segments shape:", input_batch.find_targets[0].segments.shape)

            # # DEBUG: Show per-image object breakdown
            # print("\n=== DEBUG: Per-Image Object Breakdown ===")
            # print(f"Total images in batch: {input_batch.img_batch.shape[0]}")
            # print(f"Stage 0 img_ids: {input_batch.find_inputs[0].img_ids}")
            # print(
            #     f"Stage 0 num_boxes (per query): {input_batch.find_targets[0].num_boxes}"
            # )
            # print(
            #     f"Total objects in stage 0 segments: {input_batch.find_targets[0].segments.shape[0]}"
            # )
            # print(
            #     f"Total boxes in stage 0 (packed): {input_batch.find_targets[0].boxes.shape[0]}"
            # )
            # print(f"is_valid_segment: {input_batch.find_targets[0].is_valid_segment}")

            # # Count which images have objects
            # img_ids_in_stage = input_batch.find_inputs[0].img_ids
            # from collections import Counter

            # img_counts = Counter(img_ids_in_stage.cpu().tolist())
            # print(f"\nObjects per image in stage 0:")
            # for img_idx in sorted(img_counts.keys()):
            #     print(f"  Image {img_idx}: {img_counts[img_idx]} object(s)")
            # print(
            #     f"\nImages with no queries in stage 0: {set(range(input_batch.img_batch.shape[0])) - set(img_counts.keys())}"
            # )
            # print("=" * 40 + "\n")

            # images = input_batch.img_batch.cpu().numpy()  # [B, C, H, W]
            # segments = (
            #     input_batch.find_targets[0].segments.cpu().numpy()
            # )  # [N, H, W] bool
            ...
            #     img_vis = (img_vis / img_vis.max() * 255).astype(np.uint8)
            #     img_vis = cv2.cvtColor(img_vis, cv2.COLOR_RGB2BGR)

            #     overlay = img_vis.copy()

            #     # Stack all masks and overlay each one
            #     for mask in segments:
            #         mask_bool = mask.astype(bool)
            #         overlay[mask_bool] = (0, 255, 0)  # green fill where mask is True

            #     # Blend
            #     img_vis = cv2.addWeighted(overlay, 0.5, img_vis, 0.5, 0)

            #     cv2.imshow(f"Image {idx} + Mask Overlay", img_vis)
            #     key = cv2.waitKey(0)
            ...
            with SAM3Output.iteration_mode(
                ...
                for i in range(batch_size_actual):
                    ...
                    all_predictions.append(
                        {
                            "pred_logits": final_outputs["pred_logits"][i]
                            .detach()
                            .cpu(),
    ...
    coco_gt_dict = create_coco_gt_from_dataset(
        val_ds, image_ids=all_image_ids, mask_resolution=192
    )
    ...
    if len(coco_predictions) > 0:
        ...
        cgf1_evaluator = CGF1Evaluator(
            gt_path=str(gt_file), iou_type="segm", verbose=True
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--nms-iou", type=float, default=0.7, help="NMS IoU threshold (default: 0.7)"
    )
    parser.add_argument(
        "--merge",
        action="store_true",
    ...
    if not args.use_base_model:
        if args.config is None or args.weights is None:
            parser.error(
                "--config and --weights are required when not using --use-base-model"
            )
