...
Multi-GPU Training:
  Single GPU:
        python train_sam3_lora_native.py --config configs/minimal_lora_config.yaml

    Multi-GPU (DDP):
        torchrun --nproc_per_node=2 train_sam3_lora_native.py --config configs/minimal_lora_config.yaml --multi-gpu

    Multi-GPU with specific GPUs:
        CUDA_VISIBLE_DEVICES=0,1 torchrun --nproc_per_node=2 train_sam3_lora_native.py --config configs/minimal_lora_config.yaml --multi-gpu
"""
...
from sam3.model.box_ops import box_xywh_to_xyxy, box_xyxy_to_cxcywh, masks_to_boxes
...
class COCOSegmentDataset(Dataset):
    ...
    def __init__(self, data_dir, split="train", resolution=1008):
        """
        Args:
    ...
    def __getitem__(self, idx):
        ...
        for i, ann in enumerate(annotations):
            ...
            prompt_box = None
            prompt_point = None
            if segment is not None and segment.any():
                # Compute prompt box from mask (xyxy absolute), then convert to normalized CxCyWH
                mask_box_xyxy = masks_to_boxes(segment.unsqueeze(0).float())[0]
                mask_box_cxcywh = box_xyxy_to_cxcywh(mask_box_xyxy)
                prompt_box = mask_box_cxcywh / float(self.resolution)

                # Compute centroid point from mask (x, y, label)
                ys, xs = torch.where(segment)
                if ys.numel() > 0:
                    cx = xs.float().mean()
                    cy = ys.float().mean()
                    prompt_point = torch.tensor(
                        [cx / float(self.resolution), cy / float(self.resolution), 1.0],
                        dtype=torch.float32,
                    )
            ...
            objects.append(obj)
            object_prompt_boxes.append(prompt_box)
            object_prompt_points.append(prompt_point)
        ...
        if len(class_to_object_ids) > 0:
            for query_text, obj_ids in class_to_object_ids.items():
                ...
                for obj_id in obj_ids:
                    prompt_box = object_prompt_boxes[obj_id]
                    if prompt_box is not None:
                        query_boxes.append(prompt_box)
                    prompt_point = object_prompt_points[obj_id]
                    if prompt_point is not None:
                        query_points.append(prompt_point)

                input_bbox = None
                input_bbox_label = None
                if query_boxes:
                    input_bbox = torch.stack(query_boxes)
                    input_bbox_label = torch.ones(len(query_boxes), dtype=torch.long)

                if query_points:
                    input_points = torch.stack(query_points).unsqueeze(0)
                else:
                    # Keep a consistent point embedding shape for collate
                    input_points = torch.empty((1, 0, 3), dtype=torch.float32)
...
def convert_predictions_to_coco_format(
    ...
    for img_id, preds in zip(image_ids, predictions_list):
        ...
        if debug and img_id == image_ids[0]:  # Debug first image only
            print(
                f"  Image {img_id}: {num_before} queries -> {len(scores)} after filtering (threshold={score_threshold})"
            )
        ...
        if merge_overlaps and len(binary_masks) > 0:
            ...
            if debug and img_id == image_ids[0]:
                print(
                    f"  Merged {num_before_merge} predictions -> {len(binary_masks)} (IoU threshold={iou_threshold})"
                )
        ...
        if len(binary_masks) > 0:
            ...
            if debug and img_id == image_ids[0]:
                ...
                print(
                    f"  Mask areas: min={mask_areas.min():.0f}, max={mask_areas.max():.0f}, mean={mask_areas.float().mean():.0f}"
                )
...
class SAM3TrainerNative:
    def __init__(self, config_path, multi_gpu=False, sam_checkpoint: str = "./asset/sam.pt"):
        ...
        self.log_freq = int(
            self.config.get("training", {}).get(
                "logging_steps", self.logging_cfg.get("log_freq", 10)
            )
        )
        self.use_wandb = (
            bool(self.logging_cfg.get("use_wandb", False)) and is_main_process()
        )
        self.wandb_run = None
        self.global_step = 0
        ...
        if self.use_wandb:
            if wandb is None:
                raise ImportError(
                    ...
                    "Install it with `pip install wandb` or set logging.use_wandb=false."
                )

            wandb_project = self.logging_cfg.get("wandb_project", "sam3_lora")
            wandb_run_name = self.logging_cfg.get("wandb_run_name")
            self.wandb_run = wandb.init(
                project=wandb_project,
                name=wandb_run_name,
                config=self.config,
                dir=str(Path(self.config["output"]["output_dir"])),
            )
            print_rank0(
                f"wandb logging enabled: project={wandb_project}, run_name={wandb_run_name}"
            )
    ...
    def train(self):
        ...
        # Read resolution from config (fallback to 1008)
        dataset_resolution = int(
            self.config.get("training", {}).get("resolution", 1008)
        )
        train_ds = COCOSegmentDataset(
            data_dir=data_dir, split="train", resolution=dataset_resolution
        )
        ...
        try:
            ...
            val_ds = COCOSegmentDataset(
                data_dir=data_dir, split="valid", resolution=dataset_resolution
            )
            if len(val_ds) > 0:
                has_validation = True
        ...
        for epoch in range(epochs):
            ...
            for batch_dict in pbar:
                ...
                if self.use_wandb and (self.global_step % self.log_freq == 0):
...
Examples:
  ...
  Single GPU (specific GPU):
    python train_sam3_lora_native.py --config configs/minimal_lora_config.yaml --device 1

  Multi-GPU (GPUs 0 and 1):
    python train_sam3_lora_native.py --config configs/minimal_lora_config.yaml --device 0 1

  Multi-GPU (GPUs 0, 2, 3):
    python train_sam3_lora_native.py --config configs/minimal_lora_config.yaml --device 0 2 3

  Multi-GPU (all 4 GPUs):
    python train_sam3_lora_native.py --config configs/minimal_lora_config.yaml --device 0 1 2 3
        """,
    )
    parser.add_argument(
        "--config",
        type=str,
        default="configs/minimal_lora_config.yaml",
    ...
    if num_devices > 1 and not is_torchrun_subprocess:
        ...
        trainer = SAM3TrainerNative(args.config, multi_gpu=multi_gpu, sam_checkpoint=args.sam_checkpoint)
        trainer.train()
