...
def _create_vision_backbone(
    ...
    # Position encoding
    position_encoding = _create_position_encoding(precompute_resolution=672)
    # ViT backbone
...
def _load_checkpoint(model, checkpoint_path):
    ...
    # ── Interpolate freqs_cis if image resolution differs from checkpoint ────
    import torch.nn.functional as F

    for key in list(sam3_image_ckpt.keys()):
        if "freqs_cis" not in key:
            continue
        ckpt_tensor = sam3_image_ckpt[key]  # [src_len, dim]
        try:
            *parts, attr = key.split(".")
            module = model
            for p in parts:
                module = getattr(module, p)
            current_tensor = getattr(module, attr)  # [tgt_len, dim]
        except AttributeError:
            continue
        if ckpt_tensor.shape == current_tensor.shape:
            continue
        src_len, dim = ckpt_tensor.shape
        tgt_len = current_tensor.shape[0]
        src_side = int(src_len**0.5)  # 72  (checkpoint @ 1152)
        tgt_side = int(tgt_len**0.5)  # 40  (your model  @  640)
        print(f"[freqs_cis] {key}: {src_side}²={src_len} → {tgt_side}²={tgt_len}")
        freqs_4d = ckpt_tensor.float().T.reshape(1, dim, src_side, src_side)
        freqs_interp = F.interpolate(
            freqs_4d, size=(tgt_side, tgt_side), mode="bilinear", align_corners=False
        )
        sam3_image_ckpt[key] = freqs_interp.reshape(dim, tgt_len).T.to(
            ckpt_tensor.dtype
        )
    # ────────────────────────────────────────────────────────────────────────
