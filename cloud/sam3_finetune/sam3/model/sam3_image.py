...
class Sam3Image(torch.nn.Module):
    ...
    def forward(self, input: BatchedDatapoint):
        ...
        if find_input.input_points is not None and find_input.input_points.numel() > 0:
            points = find_input.input_points
            if points.dim() == 2:
                points = points.unsqueeze(0)
            if points.dim() != 3 or points.shape[-1] < 2:
                raise ValueError(
                    f"input_points must have shape [B, N, 2/3], got {points.shape}"
                )

            # Expect input_points as [B, N, 3] -> (x, y, label)
            point_coords = points[..., :2]
            if points.shape[-1] >= 3:
                point_labels_raw = points[..., 2]
            else:
                point_labels_raw = torch.ones_like(point_coords[..., 0])

            # Convert to sequence-first for geometry encoder: [N, B, 2]
            point_embeddings = point_coords.transpose(0, 1)
            point_labels = point_labels_raw.transpose(0, 1).long()
